...
const evidenceFileFilter = (req, file, cb) => {
  ...
  if (allowedTypes.includes(file.mimetype)) {
    ...
    cb(new Error('Only JPEG, PNG, WEBP images or PDF files are allowed as evidence.'), false);
  }
};
...
function buildEvidenceUrl(filename) {
  return `/uploads/evidence/${filename}`;
}

module.exports = { upload, buildFileUrl, uploadResource, buildResourceUrl, uploadProfilePicture, buildProfilePictureUrl,uploadEvidence, buildEvidenceUrl, };